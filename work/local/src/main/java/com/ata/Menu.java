...
public class Menu {
     ...
     * @param shop takes in shop object
     */
    ...
    public void executeMenu() {
        ...
        while (!finished) {
            printMenu();
            int menuChoice = getNextIntFromUser();
            switch (menuChoice) {
                case 0:
                    finished = true;
                    exit();
                ...
                case 5:
                    ...
                    break;
                default:
                    System.out.printf("Option %d was selected. Not yet implemented%n", menuChoice);
                    break;
            }
        }
    }
    ...
    private void printMenu() {
        ...
        for (MenuOption option : MenuOption.values()) {
            System.out.printf("%d: %s%n", option.getId(), option.getDisplayName());
        }
    }
    ...
    private void findProduct() {
        ...
        int productIDResult = shop.findProduct(itemToFind);
        if (productIDResult == -1) {
            System.out.println("That product was not found.");
        } else {
            System.out.printf("%s was found and its product id is %d%n",
                    itemToFind, productIDResult);
        }
    }

    private void buyProduct() {
        System.out.println("Please enter the ID of the product you would like to purchase:");
        int iDToFind = getNextIntFromUser();
        int indexOfProduct = shop.findId(iDToFind);
        if (indexOfProduct == -1) {
            System.out.println("That item ID is invalid and could not be added to the cart.");
        } else {
            String productName = shop.addToCart(indexOfProduct);
            System.out.printf("%s has been added to your cart.%n", productName);
        }
    }
